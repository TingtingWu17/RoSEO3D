function SM_est = RoSEO3D_with_register(SMLM_img, b, n1,FPSFx, FPSFy, imgPara,varargin)
...
for ii = 1:size(lc_max,2)  
    ...
    [gammanew_cur,loc_rec_cur,NLL] =  FIST_optimize_with_register_v2(lc_cur,reshape(SMLM_img_crop,[],1),reshape(b_crop,[],1),imgPara);
    %if NLL<0.1
...
[gamma_est,loc_rec_est] = FIST_optimize_step2_with_register_v2(gamma_est,loc_rec_est,reshape(SMLM_img,[],1),reshape(b,[],1),imgPara);


SM_est = get_loc_data2_w_register1(gamma_est, loc_rec_est);     
end
